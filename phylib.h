...
// a ball that is still has a number and pos on the table
typedef struct {
unsigned char number;
phylib_coord pos;
} phylib_still_ball;

// a ball that is rolling has a number, pos on table, velocity and (negative) acceleration
typedef struct {
unsigned char number;
phylib_coord pos;
phylib_coord vel;
...
// one of the 6 holes on the table, has a pos
typedef struct {
phylib_coord pos;
} phylib_hole;
...
// vertical cushion, only has x-coordinate
typedef struct {
double x;
} phylib_vcushion;

// polymorphic parent class of objects on table
// c union that can store any class/struct in the same space
typedef union {
...
// above union cannot identify class of object, another struct is needed
typedef struct {
phylib_obj type;
phylib_untyped obj;
} phylib_object;
